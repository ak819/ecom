            ...
            <div id="add_cat" <?= ($flag=='edit')?"style='display:block'":"style='display:none'"?> >
             <div class="card">
                            <div class="card-body">
                                 ...
                                 <form class="form-horizontal" action="<?= base_url();?>ipanel/subcategory2/<?= ($flag=="edit")?"update/".$this->uri->segment(4):"store" ?>" method="post" autocomplete="off" enctype="multipart/form-data">

                                <div class="form-group row">
                                    <label class="col-md-3 m-t-15">Select Category</label>
                                    <div class="col-md-8">
                                        <select class="select2 form-control custom-select" style="width: 80%; height:36px;"  name="cat_id" onchange="getsubcat(this.value);" required>
                                    ...
                                    <div class="col-md-8">
                                        <select class="select2 form-control custom-select" style="width: 80%; height:36px;" name="sub_id" id="sub_id1" required >
                                            <option value="">Select</option>
                                            <?php if($flag == "edit") {
                                             $subcatgories=$this->m_subcategory1->getcatwise($info->cat_id);
                                             ?>
                                              <?php foreach($subcatgories as $val) { ?>
                                            <option value="<?= $val->id ?>" <?php if($flag == "edit" AND $info->subcat_id == $val->id ){ echo "selected";  } ?>><?= $val->name ?></option>
                       ...
                       <button type="button" class="btn btn-primary button_class"  id="new" onclick="add_cat();">New</button>
                        ...
                        <?php } ?>
                            ...
                            <div class="container-fluid">
                                <div class="card">
                            <div class="card-body">
                                <h4 class="card-title">Sub Category 2 List</h4>
                                <div class="table-responsive">
                                   <table id="zero_config" class="table table-striped table-bordered">
                                        <thead>
                                         ...
                                         <?php $i=1; foreach($sub_cat2_list as $val) { ?>
                                            <tr>
                                                <td><?= $i++; ?></td>
                                                <td><?= $val->cat_name ?></td>
                                                 <td><?= $val->subcat1 ?></td>
                                                 <td><?= $val->name ?></td>
                                                <td><a href="<?= base_url();?>ipanel/Subcategory2/edit/<?= $val->id;?>" class="btn btn-cyan btn-sm" data-original-title="Edit" >Edit</a>
                          <?php if ($val->status =='a'){ ?>
                          <a href="<?= base_url();?>ipanel/Subcategory2/status/d/<?= $val->id;?>" class="btn btn-success btn-sm" data-original-title="Click to Deactivate">Activated</a>
                        <?php } else { ?> <a href="<?= base_url();?>ipanel/Subcategory2/status/a/<?= $val->id;?>" class="btn btn-danger btn-sm" data-original-title="Click to Activate">Deactivated</a> <?php } ?></td>
                                            </tr>
...
function close_unit()
{
  ...
  $('.button_class').show('display','block');
   var url="<?= base_url()?>ipanel/subcategory2";
