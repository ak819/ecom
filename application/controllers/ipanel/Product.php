...
class Product extends CI_Controller {
	...
	$this->load->model(array('ipanel/M_products','ipanel/m_category','ipanel/m_brand','ipanel/m_subcategory','ipanel/m_subcategory1','ipanel/m_subcategory2','ipanel/m_subcategory3','ipanel/m_company','ipanel/m_volume'));

	//$this->products_upload_path = realpath(APPPATH.'../assets/ipanel/uploads/product_img');
	$this->image_upload_path=realpath(APPPATH.'../assets/ipanel/uploads/product_img');
	$this->video_upload_path=realpath(APPPATH.'../assets/ipanel/uploads/product_video');
	$this->pdf_upload_path=realpath(APPPATH.'../assets/ipanel/uploads/product_pdf');
	//checkLogin();
    }
    ...
    public function excelimport()
    {
       ...
       $data['brand_list']=$this->m_brand->getActive();
        ...
        $this->load->view('ipanel/product/excelimport',$data);
	...
	public function index()
	{
		...
		$data['productsList']=$this->M_products->getAll();
...
/*	   echo "<pre>";
		...
		$data['cat_list']=$this->m_category->getactiveipanel();
        $data['company_list']=$this->m_company->getActive();
        $data['volume_list']=$this->m_volume->getActive();
          
		

		$this->load->view('ipanel/header');
        $this->load->view('ipanel/sidebar');
        $this->load->view('ipanel/product/product_list',$data);
        $this->load->view('ipanel/footer');		
	...
	{
		// get the 
		$data['info']=$this->M_products->getdetails($id);
		$data['cat_list']=$this->m_category->getactiveipanel();
		$data['brand_list']=$this->m_brand->getActive();
		$data['company_list']=$this->m_company->getActive();
        $data['volume_list']=$this->m_volume->getActive();
	...
	public function update($id)
	{
		...
		{
			$file_path=$this->input->post('hidden_pdf'); 
		}
         
        
		$result=$this->M_products->update($id,$file_path);
	...
	public function importfromxls()
	{
      ...
      public function storeimg()
     {
          ...
          if($_FILES['file']['name'])
       {
        ...
        resize_image($file_path,$thumb_path,350,350);//275,184
        $small_path = $this->image_upload_path.'/small/';
        resize_image($file_path,$small_path,150,150);
      ...
      //echo "id".$id;exit;
       ...
       public function deleteImg($name,$id)
       {
		...
		$thumb_filename = $this->image_upload_path."\\"."thumb\\".$name;
       	$small_filename = $this->image_upload_path."\\"."small\\".$name; 
       	$filename = $this->image_upload_path."\\".$name; 
     ...
     public function removevideo()
      ...
      //echo "id".$id;exit;
         ...
         public function deleteimages()
       {
          ...
          $result=$this->M_products->deleteimages();
          if(!$result)
		{
			$this->session->set_flashdata('msg','Something went Worng !');
			redirect($_SERVER['HTTP_REFERER']);		
